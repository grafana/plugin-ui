...
/**
 * Bundled datasource config schemas, used as a fallback when a schema cannot be
 * fetched from the Grafana plugins CDN (for example, when the CDN returns 404).
 */
export const staticSchemaRegistry = new Map<string, DatasourceConfigSchema>([
...
/**
 * Returns the bundled fallback schema for a plugin type, or `null` if none is
 * bundled.
 */
export function getStaticConfigSchema(pluginType: string): DatasourceConfigSchema | null {
  return staticSchemaRegistry.get(pluginType) ?? null;
}
